...
let tickerCache: Record<string, { cik: string; name: string }> | null = null;
...
async function resolveToCIK(identifier: string): Promise<{ cik: string; name: string } | null> {
...
async function getCompanyFilings(identifier: string, formType?: string, count: number = 10): Promise<string> {
  const resolved = await resolveToCIK(identifier);
  if (!resolved) return `Could not resolve "${identifier}" to a CIK number. Try a ticker symbol (e.g., AAPL) or CIK number.`;
  ...
  try {
    ...
    if (!recent) return "No filing data found.";

    const results: string[] = [];
    results.push(`Company: ${data.name || resolved.name}`);
    results.push(`CIK: ${resolved.cik}`);
    if (data.tickers?.length) results.push(`Tickers: ${data.tickers.join(", ")}`);
    if (data.exchanges?.length) results.push(`Exchanges: ${data.exchanges.join(", ")}`);
    if (data.sic) results.push(`SIC: ${data.sic} - ${data.sicDescription || ""}`);
    if (data.stateOfIncorporation) results.push(`State: ${data.stateOfIncorporation}`);
    results.push("");
    ...
    for (let i = 0; i < filingCount && matched < count; i++) {
      ...
      if (formType && form !== formType && form !== `${formType}/A`) continue;
...
async function getCompanyFacts(identifier: string, concept?: string): Promise<string> {
  const resolved = await resolveToCIK(identifier);
  if (!resolved) return `Could not resolve "${identifier}" to a CIK number.`;
  ...
  try {
    if (concept) {
      ...
      for (const [taxonomy, concepts] of Object.entries(data.facts || {})) {
        const conceptObj = concepts as Record<string, any>;
...
async function getTreasuryRates(year?: number, month?: number, rateType: string = "yield_curve"): Promise<string> {
  ...
  try {
    ...
    while ((match = entryRegex.exec(raw)) !== null) {
      entryCount++;
      if (entryCount > 20) continue; // limit output
    ...
    if (entryCount > 20) results.push(`(Showing most recent 20)`);
    results.push("");
...
const POPULAR_SERIES: Record<string, { id: string; desc: string }> = {
...
async function getLaborStatistics(
  seriesId?: string,
  preset?: string,
  startYear?: number,
  endYear?: number,
  ...
  try {
    ...
    if (!series) return "No data returned.";
    ...
    for (const entry of entries) {
      ...
      const note = entry.footnotes?.map((f: any) => f.text).filter(Boolean).join("; ") || "";
...
async function tickerLookup(query: string): Promise<string> {
  const map = await loadTickerMap();
  ...
  if (matches.length === 0) return `No companies found matching "${query}".`;
...
async function economicOverview(): Promise<string> {
  const results: string[] = [];
  ...
  try {
    ...
    const raw = await httpGet(
      `https://home.treasury.gov/resource-center/data-chart-center/interest-rates/pages/xml?data=daily_treasury_yield_curve&field_tdr_date_value_month=${year}${mm}`,
